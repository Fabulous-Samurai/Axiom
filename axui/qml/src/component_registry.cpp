...
void ComponentRegistry::registerComponent(const std::string& axui_name,
                                          ComponentMeta meta) {
...
std::optional<ComponentMeta> ComponentRegistry::getMeta(
    const std::string& axui_name) const {
...
std::optional<ComponentFactory> ComponentRegistry::getFactory(
    const std::string& axui_name) const {
...
void ComponentRegistry::registerAllToEngine(QQmlEngine* engine) {
  ...
  for (const auto& [name, meta] : meta_registry_) {
    qDebug() << "Registering component:" << meta.qml_type << "from"
             << meta.qml_module;
  }
}
...
void ComponentRegistry::loadBuiltinComponents() {
  ...
  registerComponent(
      ...
      {.qml_type = "AXButton",
       ...
       .required_props = {"text"},
       .defaults = {{"variant", std::string("primary")}, {"disabled", false}},
       .supports_glass = true,
       .supports_hover = true});
  ...
  registerComponent(
      ...
      {.qml_type = "AXTextField",
       ...
       .required_props = {},
       .defaults = {{"placeholder", std::string("")}, {"disabled", false}},
       .supports_glass = true,
       .supports_hover = true});

  registerComponent(
      "Select", {.qml_type = "AXSelect",
                 .qml_module = "AXUI.Components",
                 .qml_file = "qrc:/axui/AXUI/Components/controls/AXSelect.qml",
                 .required_props = {"options"},
                 .defaults = {{"placeholder", std::string("Select...")},
                              {"searchable", false}},
                 .supports_glass = true,
                 .supports_hover = true});
  ...
  registerComponent(
      ...
      {.qml_type = "AXToast",
       ...
       .defaults = {{"type", std::string("info")}, {"duration", 3000.0}},
       .supports_glass = true,
       .supports_hover = false});
  ...
  registerComponent(
      ...
      {.qml_type = "AXSkeleton",
       ...
       .required_props = {},
       .defaults = {{"variant", std::string("rect")}, {"animated", true}},
       .supports_glass = false,
       .supports_hover = false});
