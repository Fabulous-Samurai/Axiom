...
bool QmlLoader::loadFromNode(const UINode& root, const QmlConfig& config) {
  ...
  if (component.isError()) {
    std::cerr << "[AXIOM UI] QML Component Error: "
              << component.errorString().toStdString() << std::endl;
    return false;
  }
...
bool QmlLoader::loadFromFile(const std::string& axui_json_path,
                             const std::string& theme_json_path,
...
QString QmlLoader::generateComponentCode(const UINode& node, int depth) {
  ...
  if (!node.id.empty()) {
    QString id_str =
        QString::fromUtf8(node.id.data(), static_cast<int>(node.id.size()));
...
QString QmlLoader::convertPropertyToQml(const Property& prop, int depth) {
  ...
  QString code;
  QString key =
      QString::fromUtf8(prop.key.data(), static_cast<int>(prop.key.size()));
  ...
  std::visit(
      [&](auto&& value) {
        ...
        if constexpr (std::is_same_v<T, double>) {
          ...
          QString path = QString::fromUtf8(value.path.data(),
                                           static_cast<int>(value.path.size()));
...
void QmlLoader::setupBindings(QQuickItem* item, const UINode& node) {
  ...
  for (const auto& prop : node.properties) {
    if (auto* binding = std::get_if<Binding>(&prop.value)) {
      if (binding->is_bound) {
        QString qml_prop = QString::fromUtf8(prop.key.data(),
                                             static_cast<int>(prop.key.size()));
        QString path = QString::fromUtf8(
            binding->path.data(), static_cast<int>(binding->path.size()));

        // [MANDATORY PATH]: Register QML callback for data flow
        connect(binding_engine_, &BindingEngine::sourceUpdated, this,
                [item, qml_prop, path](const QString& updated_path,
                                       const BindingValue& val) {
                  if (updated_path == path) {
                    // Update QML property dynamically
                    // [MANDATORY]: Use QVariant::fromValue for complex types in
                    // Zenith
                    item->setProperty(qml_prop.toUtf8().constData(),
  ...
  for (int i = 0; i < std::min((int)children.size(), (int)node.children.size());
       ++i) {
