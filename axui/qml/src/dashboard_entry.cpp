...
int main(int argc, char *argv[]) {
    ...
    if (mockMode) {
        ...
        if (port == 0) port = 5555;
        std::cout << "[Dashboard] Connecting to AXIOM Engine on port " << port << std::endl;
    }
    ...
    QObject::connect(&engine, &QQmlApplicationEngine::objectCreated,
        ...
        if (!obj && url == objUrl) {
            std::cerr << "[Dashboard] CRITICAL: QML Engine failed to create object for " << url.toString().toStdString() << std::endl;
    ...
    if (engine.rootObjects().isEmpty()) {
        ...
        std::cout << "[Dashboard] Dashboard ready. Executing app loop..." << std::endl;
    }
