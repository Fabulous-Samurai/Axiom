...
void test_builtin_components_registered() {
  ...
  auto hasComponent = [&](const std::string& name) {
    return std::find(components.begin(), components.end(), name) !=
           components.end();
  };
...
void test_component_meta_retrieval() {
  ...
  assert(std::find(required.begin(), required.end(), "title") !=
         required.end());
  assert(std::find(required.begin(), required.end(), "value") !=
         required.end());
