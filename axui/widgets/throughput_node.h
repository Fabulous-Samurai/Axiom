 ...
 * 2. L1/L2 CACHE OPTIMIZED: Vertex updates are performed in a contiguous memory
 * block to maximize spatial locality and minimize TLB misses.
 * 3. GIL BYPASS: The SceneGraph thread (Data Plane) handles the vertex upload
 * independently of the Python UI thread (Control Plane).
 */
class AxiomThroughputNode : public QSGGeometryNode {
 ...
 private:
  ...
  // Aligned to 64 bytes to prevent bank conflicts during high-frequency
  // updates.
...
class AxiomThroughputItem : public QQuickItem {
  ...
  Q_PROPERTY(
      int maxPoints READ maxPoints WRITE setMaxPoints NOTIFY maxPointsChanged)
