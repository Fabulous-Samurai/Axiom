...
class PlutoSwarmNode : public QSGGeometryNode {
 public:
  PlutoSwarmNode();
  void update(const QRectF& rect, size_t count, const QPointF* positions,
...
/**
 * @brief PlutoSwarmNavigator: QML wrapper for the Pluto search tree
 * visualization.
 */
class PlutoSwarmNavigator : public QQuickItem {
  Q_OBJECT
  Q_PROPERTY(QColor nodeColor READ nodeColor WRITE setNodeColor NOTIFY
                 nodeColorChanged)
  Q_PROPERTY(QColor edgeColor READ edgeColor WRITE setEdgeColor NOTIFY
                 edgeColorChanged)
  Q_PROPERTY(
      int selectedNodeId READ selectedNodeId NOTIFY selectedNodeIdChanged)
 ...
 public:
  ...
  void setNodeColor(const QColor& c) {
    if (m_nodeColor != c) {
      m_nodeColor = c;
      Q_EMIT nodeColorChanged();
      update();
    }
  }
 ...
 private:
  ...
  QColor m_edgeColor =
      QColor::fromRgbF(125 / 255.0, 211 / 255.0, 252 / 255.0, 0.3);
