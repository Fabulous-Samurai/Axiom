...
namespace {
    void myMessageOutput(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
        ...
        switch (type) {
            case QtDebugMsg:   prefix = "[Qt Debug] "; break;
...
int AppRunner::run(const std::string& main_qml_path, const std::string& window_title) {
    ...
    QObject::connect(&engine_, &QQmlApplicationEngine::objectCreated,
        ...
        if (!obj && url == objUrl) {
            std::cerr << "[AppRunner] CRITICAL: QML Engine failed to create object for " << url.toString().toStdString() << std::endl;
    ...
    if (engine_.rootObjects().isEmpty()) {
        std::cerr << "[AppRunner] ERROR: Root objects are empty after engine.load()" << std::endl;
        return -1;
    }
    ...
    std::cout << "[AppRunner] " << app_.applicationName().toStdString() << " ready. Executing app loop..." << std::endl;
    return app_.exec();
}
