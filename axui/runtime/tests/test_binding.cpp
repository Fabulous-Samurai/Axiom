...
void test_binding_singleton() {
  ...
  if (&e1 != &e2) {
    std::cerr << "Singleton failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_single_update() {
  ...
  if (!is_near(std::get<double>(*prop), 45.5)) {
...
void test_binding_multiple_children() {
  ...
  if (!is_near(std::get<double>(*cpu), 10.0)) {
    std::cerr << "CPU mismatch!" << std::endl;
    exit(1);
  }
  if (!is_near(std::get<double>(*ram), 85.0)) {
    std::cerr << "RAM mismatch!" << std::endl;
    exit(1);
  }
}
...
void test_binding_clear_and_rebind() {
  ...
  auto result =
      compiler.compile(R"({"root":{"component":"Text","props":{"v":"@t"}}})");
  engine.bindTree(*result.root);
  if (engine.subscriptionCount() != 1) {
    std::cerr << "Sub count mismatch!" << std::endl;
    exit(1);
  }
...
void test_binding_source_query() {
  ...
  if (!engine.hasSource("query.test")) {
    std::cerr << "Source not found!" << std::endl;
    exit(1);
  }
  if (std::get<double>(engine.getSource("query.test")) != 123.45) {
    std::cerr << "Value mismatch!" << std::endl;
    exit(1);
  }
}
...
void test_binding_statistics() {
  ...
  if (s.total_updates == 0) {
    std::cerr << "Stats failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_theme_tokens_untouched() {
  ...
  auto result = compiler.compile(
      R"({"root":{"component":"Text","props":{"color":"#FF0000"}}})");
  engine.bindTree(*result.root);
  if (engine.subscriptionCount() != 0) {
    std::cerr << "Theme token bound incorrectly!" << std::endl;
    exit(1);
  }
}
...
void test_binding_late_binding() {
  ...
  Compiler compiler;
  auto result = compiler.compile(
      R"({"root":{"component":"Text","props":{"v":"@late.val"}}})");
  engine.bindTree(*result.root);

  auto* p = result.root->getProperty("v");
  if (std::get<double>(*p) != 99.0) {
    std::cerr << "Late binding failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_int64_promotion() {
  ...
  Compiler compiler;
  auto result = compiler.compile(
      R"({"root":{"component":"Text","props":{"v":"@int.val"}}})");
  engine.bindTree(*result.root);

  auto* p = result.root->getProperty("v");
  if (std::get<double>(*p) != 42.0) {
    std::cerr << "Promotion failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_color_value() {
  ...
  Compiler compiler;
  auto result = compiler.compile(
      R"({ "root": { "component": "Text", "props": { "c": "@color.val" } } })");
  engine.bindTree(*result.root);

  auto* p = result.root->getProperty("c");
  if (std::get<Color>(*p).r != 255) {
    std::cerr << "Color binding failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_bool_value() {
  ...
  Compiler compiler;
  auto result = compiler.compile(
      R"({ "root": { "component": "Text", "props": { "b": "@bool.val" } } })");
  engine.bindTree(*result.root);

  auto* p = result.root->getProperty("b");
  if (std::get<bool>(*p) != true) {
    std::cerr << "Bool binding failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_string_value() {
  ...
  Compiler compiler;
  auto result = compiler.compile(
      R"({ "root": { "component": "Text", "props": { "s": "@str.val" } } })");
  engine.bindTree(*result.root);

  auto* p = result.root->getProperty("s");
  if (std::get<std::string_view>(*p) != "hello") {
    std::cerr << "String binding failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_batch_update() {
  ...
  BindingEngine& engine = BindingContext::instance();
  std::vector<SourceUpdate> updates = {{"batch.1", 1.0}, {"batch.2", 2.0}};
  engine.updateBatch(updates);
  if (std::get<double>(engine.getSource("batch.1")) != 1.0) {
    std::cerr << "Batch 1 failed!" << std::endl;
    exit(1);
  }
}
...
void test_binding_shared_path() {
  ...
  if (std::get<double>(*result.root->children[0]->getProperty("v")) != 5.5) {
    std::cerr << "Shared path failed!" << std::endl;
    exit(1);
  }
}
