...
DashboardManager::DashboardManager(QObject* parent) : QObject(parent) {
  update_timer_ = new QTimer(this);
  update_timer_->setInterval(UPDATE_INTERVAL_MS);
  connect(update_timer_, &QTimer::timeout, this,
          &DashboardManager::onUpdateTick);
  update_timer_->start();
}
...
void DashboardManager::registerStage(std::string_view stage_id,
                                     std::string_view display_name,
                                     int order) noexcept {
  std::lock_guard lock(registry_mutex_);

  auto it = std::ranges::find_if(
      stages_, [&](const auto& p) { return p.first == stage_id; });
  if (it == stages_.end()) {
...
void DashboardManager::registerLink(std::string_view source_id,
                                    std::string_view target_id) noexcept {
  std::lock_guard lock(registry_mutex_);
...
void DashboardManager::recordMessage(std::string_view stage_id,
                                     uint64_t byte_count) noexcept {
  // Binary search or direct indexing would be better, but linear search on 32
  // items is fast.
  for (auto& [id, s] : stages_) {
    if (id == stage_id) {
      ...
      s->last_activity.store(std::chrono::steady_clock::now(),
...
void DashboardManager::recordLatency(std::string_view stage_id,
                                     double latency_us) noexcept {
  for (auto& [id, s] : stages_) {
    if (id == stage_id) {
      s->avg_latency_us.store(
          latency_us, std::memory_order_seq_cst);  // Simplified for audit
      return;
    }
  }
}

void DashboardManager::updateStageState(std::string_view stage_id,
...
void DashboardManager::updateQueueStatus(std::string_view stage_id,
                                         uint64_t current,
  ...
  for (auto& [id, s] : stages_) {
    if (id == stage_id) {
      ...
      if (capacity > 0) {
        ...
        if (ratio > 0.9)
          s->state.store(StageState::Blocked, std::memory_order_seq_cst);
      }
      return;
    }
  }
...
void DashboardManager::reportError(std::string_view stage_id,
                                   std::string_view error_msg) noexcept {
  for (auto& [id, s] : stages_) {
    if (id == stage_id) {
...
void DashboardManager::calculateThroughput() noexcept {
  ...
  for (auto& [id, s] : stages_) {
    ...
    if (uptime > 0)
      s->messages_per_second.store(static_cast<double>(msgs) / uptime,
                                   std::memory_order_seq_cst);
  }
}
...
void DashboardManager::collectSystemTelemetry() {
  ...
  system_telemetry_.cpu_usage_percent.store(
      (pmu.cycles > 0)
          ? std::min(100.0, (double)pmu.instructions / pmu.cycles * 50.0)
          : 10.0,
      std::memory_order_seq_cst);
  system_telemetry_.ipc_bytes_per_sec.store(
      AXIOM::TelemetryScribe::instance().read_throughput(),
      std::memory_order_seq_cst);

  auto mem_stats = AXIOM::MemoryOrchestrator::instance().get_system_stats();
  system_telemetry_.ram_total_bytes.store(mem_stats.total_reserved_bytes,
                                          std::memory_order_seq_cst);
  system_telemetry_.ram_used_bytes.store(mem_stats.total_used_bytes,
                                         std::memory_order_seq_cst);
  if (mem_stats.total_reserved_bytes > 0) {
    system_telemetry_.ram_usage_percent.store(
        static_cast<double>(mem_stats.total_used_bytes) /
            mem_stats.total_reserved_bytes * 100.0,
        std::memory_order_seq_cst);
  }
}
...
QVariantList DashboardManager::stagesModel() const {
  ...
  for (const auto& [id, s] : stages_) {
    ...
    map["name"] =
        QString::fromUtf8(s->display_name.data(), s->display_name.size());
    map["state"] = static_cast<int>(s->state.load(std::memory_order_seq_cst));
    map["totalMessages"] = static_cast<qulonglong>(
        s->total_messages.load(std::memory_order_seq_cst));
    map["messagesPerSecond"] =
        s->messages_per_second.load(std::memory_order_seq_cst);
...
QVariantList DashboardManager::linksModel() const {
  ...
  for (const auto* l : links_) {
    QVariantMap map;
    map["sourceId"] =
        QString::fromUtf8(l->source_id.data(), l->source_id.size());
    map["targetId"] =
        QString::fromUtf8(l->target_id.data(), l->target_id.size());
...
QVariantMap DashboardManager::telemetryModel() const {
  QVariantMap map;
  map["cpuUsage"] =
      system_telemetry_.cpu_usage_percent.load(std::memory_order_seq_cst);
  map["ramUsage"] =
      system_telemetry_.ram_usage_percent.load(std::memory_order_seq_cst);
  map["ipcBps"] =
      system_telemetry_.ipc_bytes_per_sec.load(std::memory_order_seq_cst);
  return map;
}
...
double DashboardManager::totalThroughput() const noexcept {
  double total = 0;
  for (const auto& [id, s] : stages_)
    total += s->messages_per_second.load(std::memory_order_seq_cst);
  return total;
}
