...
class DashboardManager : public QObject {
  ...
  Q_PROPERTY(
      QVariantMap systemTelemetry READ telemetryModel NOTIFY telemetryUpdated)
 ...
 public:
  ...
  void registerStage(std::string_view stage_id, std::string_view display_name,
                     int order) noexcept;
  ...
  void updateQueueStatus(std::string_view stage_id, uint64_t current,
 ...
 private:
  ...
  // [ZENITH]: Multi-producer registry using minimal mutex for rare write
  // operations
  ...
  AXIOM::FixedVector<std::pair<std::string_view, ThroughputVec*>, 32>
      throughput_history_;
  AXIOM::FixedVector<std::pair<std::string_view, LatencyVec*>, 32>
      latency_history_;
