...
void ThemeResolver::loadTheme(const std::string& theme_json) {
  ...
  try {
    std::string padded_json =
        theme_json + std::string(simdjson::SIMDJSON_PADDING, ' ');
      ...
      if (key == "name") {
        ...
        for (auto tfield : typo_obj) {
          ...
          if (ttype == simdjson::ondemand::json_type::number) {
            ...
            theme_.typographyStrings[tkey] =
                std::string(tval.get_string().value());
          }
        }
      } else if (key == "spacing") {
        ...
        for (auto efield : elev_obj) {
          ...
          for (auto lfield : level_obj) {
            ...
            if (lkey == "offset")
              level.offset = lfield.value().get_double().value();
            else if (lkey == "blur")
              level.blur = lfield.value().get_double().value();
            else if (lkey == "opacity")
              level.opacity = lfield.value().get_double().value();
          }
        ...
        for (auto gfield : glass_obj) {
          ...
          if (gkey == "defaultBlur")
            theme_.glass.defaultBlur = gfield.value().get_double().value();
          else if (gkey == "defaultOpacity")
            theme_.glass.defaultOpacity = gfield.value().get_double().value();
          else if (gkey == "defaultBorderOpacity")
            theme_.glass.defaultBorderOpacity =
                gfield.value().get_double().value();
          else if (gkey == "defaultNoiseStrength")
            theme_.glass.defaultNoiseStrength =
                gfield.value().get_double().value();
        }
        ...
        for (auto afield : anim_obj) {
          ...
          if (akey == "fast")
            theme_.animation.fast = afield.value().get_double().value();
          else if (akey == "normal")
            theme_.animation.normal = afield.value().get_double().value();
          else if (akey == "slow")
            theme_.animation.slow = afield.value().get_double().value();
        }
      }
    }
    loaded_ = true;
...
double ThemeResolver::resolveNumber(const std::string& token) const {
  ...
  if (parts.category == "glass") {
    ...
    if (parts.key == "defaultBorderOpacity")
      return theme_.glass.defaultBorderOpacity;
    if (parts.key == "defaultNoiseStrength")
      return theme_.glass.defaultNoiseStrength;
  }
  return 0.0;
}
...
size_t ThemeResolver::tokenCount() const {
  return theme_.colors.size() + theme_.typography.size() +
         theme_.typographyStrings.size() + theme_.spacing.size() +
         theme_.radius.size() + theme_.elevation.size();
}
        ...
        if (parts.category == "colors") {
          ...
          if (num_it != theme_.typography.end()) {
            ...
            if (str_it != theme_.typographyStrings.end()) {
              prop.value = std::string_view(
                  str_it->second);  // Note: lifetime must be managed
              resolve_count_++;
            }
          }
        } else if (parts.category == "spacing" || parts.category == "radius" ||
                   parts.category == "animation" || parts.category == "glass") {
  ...
  if (glass.enabled && loaded_) {
    ...
    if (glass.border_opacity == 0.2f)
      glass.border_opacity =
          static_cast<float>(theme_.glass.defaultBorderOpacity);
    if (glass.noise_strength == 0.015f)
      glass.noise_strength =
          static_cast<float>(theme_.glass.defaultNoiseStrength);
  }
}
