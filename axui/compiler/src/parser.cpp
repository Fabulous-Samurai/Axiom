...
UINode* Parser::parse(const std::string& json_content,
                      AXIOM::ArenaAllocator<UINode>& arena) {
  errors_.clear();
...
void Parser::parseNodeInto(simdjson::ondemand::object& obj, UINode& node,
                           AXIOM::ArenaAllocator<UINode>& arena) {
  for (auto field : obj) {
    ...
    if (key == "component") {
      node.component_type =
          componentTypeFromString(field.value().get_string().value());
    } else if (key == "id") {
...
GlassParams Parser::parseGlass(simdjson::ondemand::object& obj) {
  ...
  for (auto field : obj) {
    auto key = field.unescaped_key().value();
    if (key == "blur")
      g.blur_radius = static_cast<float>(field.value().get_double().value());
    else if (key == "opacity")
      g.bg_opacity = static_cast<float>(field.value().get_double().value());
    else if (key == "borderOpacity")
      g.border_opacity = static_cast<float>(field.value().get_double().value());
  }
  return g;
}
...
HoverParams Parser::parseHover(simdjson::ondemand::object& obj) {
  ...
  for (auto field : obj) {
    auto key = field.unescaped_key().value();
    if (key == "scale")
      h.scale = static_cast<float>(field.value().get_double().value());
    else if (key == "glowRadius")
      h.glow_radius = static_cast<float>(field.value().get_double().value());
  }
  return h;
}
...
LayoutParams Parser::parseLayout(simdjson::ondemand::object& obj) {
  ...
  for (auto field : obj) {
    ...
    if (key == "type") {
      ...
      if (t == "row")
        l.type = LayoutType::Row;
      else if (t == "column")
        l.type = LayoutType::Column;
    } else if (key == "width")
      l.width = static_cast<float>(field.value().get_double().value());
    else if (key == "height")
      l.height = static_cast<float>(field.value().get_double().value());
    else if (key == "gap")
      l.gap = static_cast<float>(field.value().get_double().value());
  }
  return l;
}

void Parser::parsePropertiesInto(simdjson::ondemand::object& obj, UINode& node,
                                 AXIOM::ArenaAllocator<UINode>& arena) {
  for (auto field : obj) {
    Property p;
    ...
    if (type == simdjson::ondemand::json_type::string) {
      ...
      if (sv.starts_with("@"))
        p.value = parseBinding(sv);
      else if (sv.starts_with("#"))
        p.value = Color::fromHex(sv);
      else
        p.value = sv;
