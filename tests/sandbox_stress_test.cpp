...
int main() {
    ...
    if (res1.success) {
        std::cout << "[SUCCESS] Result: " << res1.result << " (Time: " << res1.execution_time_ms << "ms)" << std::endl;
    } else {
    ...
    // We'll send a command that the calculator might take time with or we just test the IPC speed.
    ...
    std::cout << "\n[TEST 3] Sending Poisonous 'STALL_INFINITE' (Timeout limit 200ms)..." << std::endl;
    ...
    if (!res3.success && res3.error.find("Stalled") != std::string::npos) {
        std::cout << "[SUCCESS] Sentry/Proxy killed the hung process. Error: " << res3.error << std::endl;
    } else {
        std::cout << "[FAILED] Self-healing failed or not triggered." << std::endl;
    }

    // TEST 4: Recovery Validation
    std::cout << "\n[TEST 4] Validating recovery: Sending '10 + 10' to the new worker..." << std::endl;
    auto fut4 = proxy.execute("10 + 10");
    auto res4 = fut4.get();

    if (res4.success && res4.result.find("20") != std::string::npos) {
        std::cout << "[SUCCESS] System fully recovered and responsive!" << std::endl;
    } else {
