...
int main() {
  ...
  std::cout << "x^2 result has_value: " << result1.result.has_value()
            << std::endl;

  // Test 2: Function with parentheses
  std::cout << "\n=== Testing function syntax ===\n";
  auto result2 = parser.ParseAndExecute("sin(45)");
  std::cout << "sin(45) result: "
            << (result2.result.has_value() ? "SUCCESS" : "FAILED") << std::endl;
  if (result2.result.has_value()) {
    std::cout << "sin(45) = " << std::get<double>(result2.result.value())
              << std::endl;
  }
  ...
  std::cout << "limit(2, x, 1) result: "
            << (result3.result.has_value() ? "SUCCESS" : "FAILED") << std::endl;
  if (result3.result.has_value()) {
    std::cout << "limit(2, x, 1) = " << std::get<double>(result3.result.value())
              << std::endl;
  ...
  std::cout << "limit(x, x, 5) result: "
            << (result4.result.has_value() ? "SUCCESS" : "FAILED") << std::endl;
  if (result4.result.has_value()) {
    std::cout << "limit(x, x, 5) = " << std::get<double>(result4.result.value())
              << std::endl;
  }
  ...
  try {
    ...
    if (result5.result.has_value()) {
      std::cout << " SUCCESS: limit(3*x, x, 2) = "
                << std::get<double>(result5.result.value()) << std::endl;
