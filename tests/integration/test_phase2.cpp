...
int main() {
  ...
  if (AXIOM::IsComplex(complex_product)) {
    ...
    std::cout << "Multiply(c1, c2) = " << cp.real() << " + " << cp.imag()
              << "i\n\n";
  }
  ...
  if (AXIOM::IsComplex(mixed_product)) {
    ...
    std::cout << "Multiply(4.0, 2i) = " << mp.real() << " + " << mp.imag()
              << "i\n\n";
  }
  ...
  auto result3 =
      CreateSuccessResult(AXIOM::Number(std::complex<double>(0.0, 1.0)));

  std::cout << "Test 5: EngineResult with Complex Support\n";
  std::cout << "Real result available: " << (result1.HasResult() ? "Yes" : "No")
            << "\n";
  std::cout << "Complex result available: "
            << (result2.HasResult() ? "Yes" : "No") << "\n";
  std::cout << "AXIOM Number result available: "
            << (result3.HasResult() ? "Yes" : "No") << "\n\n";
