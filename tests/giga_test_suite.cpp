...
class TestRunner {
 ...
 public:
  TestRunner() { start_time = std::chrono::steady_clock::now(); }
  ...
  void EndSection() {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                        end - section_start_time)
                        .count();
    std::cout << Color::CYAN << "Section completed in " << duration << "ms"
              << Color::RESET << "\n";
  }
  ...
  void RunTest(const std::string& test_name, Func test_func) {
    ...
    try {
      ...
      failed_tests++;
      failed_test_names.push_back(test_name);
      std::cout << Color::RED << "FAIL (runtime error: " << e.what() << ")"
                << Color::RESET << "\n";
    } catch (const std::exception& e) {
      failed_tests++;
      failed_test_names.push_back(test_name);
      std::cout << Color::RED << "FAIL (exception: " << e.what() << ")"
                << Color::RESET << "\n";
    }
  }
  ...
  int GetExitCode() const { return (failed_tests == 0) ? 0 : 1; }
};
...
void TestAlgebraicParser(TestRunner& runner) {
  ...
  runner.RunTest("sin(30) calculation", [&parser]() {
    ...
    return result.HasResult() &&
           approx_equal(*result.GetDouble(), 0.5);  // sin(30°) = 0.5
  });
  ...
  runner.RunTest("log(100) calculation", [&parser]() {
    ...
    return result.HasResult() &&
           approx_equal(*result.GetDouble(), std::log10(100.0));
  });
...
void TestUnitManager(TestRunner& runner) {
  ...
  runner.RunTest("Unit compatibility: m and km",
                 [&units]() { return units.AreCompatible("m", "km"); });

  // Test 5: Incompatible units
  runner.RunTest("Unit incompatibility: kg and m",
                 [&units]() { return !units.AreCompatible("kg", "m"); });
...
void TestEigenEngine(TestRunner& runner) {
  ...
  runner.RunTest("Matrix inverse", [&eigen]() {
    ...
    auto A_inv_opt = eigen.Inverse(A);
    if (!A_inv_opt) return false;
  ...
  runner.RunTest("FFT magnitude of impulse", [&eigen]() {
    AXIOM::EigenEngine::Vector input_signal =
        AXIOM::EigenEngine::Vector::Zero(4);
    input_signal[0] = 1.0;
    auto fft = eigen.FFT(input_signal);
    return fft.size() == 4 && approx_equal(fft(0), 1.0, 1e-6) &&
           approx_equal(fft(3), 1.0, 1e-6);
  });

  // Test 9: IFFT restoration from DC spectrum
  runner.RunTest("IFFT from DC-only spectrum", [&eigen]() {
    AXIOM::EigenEngine::Vector freq_spectrum =
        AXIOM::EigenEngine::Vector::Zero(4);
    freq_spectrum[0] = 4.0;
    auto restored = eigen.IFFT(freq_spectrum);
    return restored.size() == 4 && approx_equal(restored(0), 1.0, 1e-6) &&
           approx_equal(restored(2), 1.0, 1e-6);
  });
  ...
  runner.RunTest("Convolution [1,2,3] * [1,1]", [&eigen]() {
    ...
    return conv.size() == 4 && approx_equal(conv(0), 1.0, 1e-6) &&
           approx_equal(conv(2), 5.0, 1e-6) && approx_equal(conv(3), 3.0, 1e-6);
  });
  ...
  runner.RunTest("Cross-correlation [1,2,3] vs [2,1]", [&eigen]() {
    ...
    return corr.size() == 4 && approx_equal(corr(0), 1.0, 1e-6) &&
           approx_equal(corr(2), 7.0, 1e-6) && approx_equal(corr(3), 6.0, 1e-6);
  });
...
void TestHarmonicArena(TestRunner& runner) {
  ...
  runner.RunTest("Concurrent lock-free allocations", []() {
    // Keep test memory budget conservative while exercising block-rotation
    // behavior.
    ...
    for (int t = 0; t < thread_count; ++t) {
      workers.emplace_back([&arena, &ok_count]() {
        for (int i = 0; i < allocs_per_thread; ++i) {
          ...
          if (p != nullptr && (reinterpret_cast<std::uintptr_t>(p) %
                               AXIOM::CACHE_LINE_SIZE) == 0) {
...
void TestAdversarialSuite(TestRunner& runner) {
  ...
  runner.RunTest("HarmonicArena Fragmentation Stress", []() {
    // Test lock-free allocator stability under erratic allocation/free-like
    // reuse patterns
  ...
  runner.RunTest("FPU Poison (NaN & Infinity Propagation)", []() {
    ...
    // Must safely trap evaluating NaN/Infinity as an error node or return no
    // result.
    return !result.HasResult();
  });
