...
static void BM_PlutoRender10k(benchmark::State& state) {
  ...
  // We can't easily mock it without affecting other tests or using dirty
  // tricks. But we can just measure the loop in PlutoSwarmNode::update by
  // copying its logic or manually filling the controller's scratchpad if it was
  // exposed.
