...
enum class NodeType { Number, Variable, BinaryOp, UnaryOp, MultiArgFunction };
...
NodePtr Parse(Arena &arena, std::string_view input) {
  ...
  if (pos == std::string_view::npos) pos = input.find('-');
  if (pos != std::string_view::npos) {
    auto left = input.substr(0, pos);
    auto right = input.substr(pos + 1);
    char op = input[pos];
    return arena.alloc<BinaryOpNode>(op, Parse(arena, left),
                                     Parse(arena, right));
  }
  ...
  if (pos != std::string_view::npos) {
    auto left = input.substr(0, pos);
    auto right = input.substr(pos + 1);
    return arena.alloc<BinaryOpNode>('*', Parse(arena, left),
                                     Parse(arena, right));
  }
  ...
  if (auto val = ToDouble(input)) {
    ...
    // CRITICAL : copy the variable name to the arena ,so even if there is any
    // input string, VariableNode still lives
