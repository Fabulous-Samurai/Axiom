 ...
 public:
  ...
  void RunTest(const std::string& name, Func test_func) {
    ...
    try {
      ...
      std::cout << "\x1b[31mFAIL\x1b[0m (runtime error: " << e.what() << ")"
                << std::endl;
    } catch (const std::exception& e) {
      std::cout << "\x1b[31mFAIL\x1b[0m (exception: " << e.what() << ")"
                << std::endl;
    }
  }
...
void TestStatisticsEngineEdgeCases(EdgeCaseRunner& runner) {
  ...
  runner.RunTest("Variance of constant values", [&stats]() {
    ...
    return result.HasResult() &&
           std::abs(*result.GetDouble()) < 0.01;  // Should be 0
  });
...
void TestLinearSystemParserEdgeCases(EdgeCaseRunner& runner) {
  ...
  runner.RunTest("Large coefficients", [&parser]() {
    auto result = parser.ParseAndExecute(
        "solve [[1000000,2000000],[3000000,4000000]] [5000000,11000000]");
  ...
  runner.RunTest("Small coefficients (near zero)", [&parser]() {
    auto result = parser.ParseAndExecute(
        "solve [[0.0001,0.0002],[0.0003,0.0004]] [0.0005,0.0011]");
...
int main() {
  ...
  std::cout
      ...
      << std::endl;
  std::cout
      << "║        AXIOM ENGINE v3.0 - EDGE CASE TEST SUITE               ║"
      << std::endl;
  std::cout
      << "║                                                               ║"
      << std::endl;
