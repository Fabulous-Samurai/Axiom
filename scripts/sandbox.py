...
def run_isolated_expression(expression):
    ...
    # to avoid shell quoting issues. We use safe_eval instead of eval() to prevent RCE.
    sandbox_dir = os.path.dirname(os.path.abspath(__file__))
    code = f"import sys; sys.path.insert(0, {repr(sandbox_dir)}); from safe_eval import evaluate; print(evaluate({repr(expression)}))"
