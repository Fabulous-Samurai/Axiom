...
def run_isolated_expression(expression):
    ...
    # What: Restrict eval() __builtins__ and set shell=False.
    # Why: Prevent arbitrary code execution and sandbox escapes via unrestricted eval().
    code = "print(eval(%s, {'__builtins__': {'abs': abs, 'min': min, 'max': max, 'int': int, 'float': float}}))" % repr(expression)
