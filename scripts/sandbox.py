...
def run_isolated_expression(expression):
    ...
    # What: Restrict eval environment with a safe whitelist for __builtins__
    # Why: Prevent arbitrary code execution through malicious expressions
    safe_env = "{'__builtins__': {'abs': abs, 'min': min, 'max': max, 'int': int, 'float': float}}"
    code = "print(eval(%r, %s))" % (expression, safe_env)
