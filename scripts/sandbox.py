...
def run_isolated_expression(expression):
    ...
    sandbox_dir = __import__("os").path.dirname(__import__("os").path.abspath(__file__))
    code = f"import sys; sys.path.insert(0, {repr(str(sandbox_dir) + '/..')}); from tools.visualization.utils.secure_eval import secure_eval; print(secure_eval({repr(expression)}))"
