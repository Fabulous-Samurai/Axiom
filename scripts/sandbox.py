...
def run_isolated_expression(expression):
    ...
    # 🛡️ SENTINEL SECURITY FIX: Restrict eval environment to prevent RCE
    code = f"print(eval({repr(expression)}, {{'__builtins__': {{'abs': abs, 'min': min, 'max': max, 'int': int, 'float': float}}}}))"
