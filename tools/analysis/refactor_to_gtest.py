...
# Convert benchmark_suite.cpp to GBenchmark
...
static void BM_ZeroCopyVector(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        large_vec.reserve(vector_size);
        std::fill_n(std::back_inserter(large_vec), vector_size, 1.0);
