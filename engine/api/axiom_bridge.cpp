...
AXIOM_EXPORT Axiom_CalculationResult Axiom_Execute(const char* expression,
  ...
  if (!expression) {
    res.status_code = -1;
    std::snprintf(res.error_msg, sizeof(res.error_msg), "%s",
                  "Null expression");
    return res;
  }
  ...
  if (engine_res.HasErrors()) {
    ...
    if (engine_res.error.has_value()) {
      std::visit(
          [&](auto& err) {
            std::snprintf(res.error_msg, sizeof(res.error_msg),
...
AXIOM_EXPORT uintptr_t AxiomJit_GetDisassembly(const uint8_t* code_ptr,
                                               size_t size,
                                               char* out_preallocated_buffer,
                                               size_t max_len) {
  if (!out_preallocated_buffer || max_len == 0) return 0;
  // ... (rest of disassembly implementation)
  int written =
      std::snprintf(out_preallocated_buffer, max_len,
                    "AXIOM_JIT_BINARY (0x%p, %zu bytes)", code_ptr, size);
  if (written < 0) return 0;
  return (static_cast<size_t>(written) < max_len)
