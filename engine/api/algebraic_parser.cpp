...
NodePtr parse_number(ParserState& state) noexcept {
  ...
  while (state.has_more() &&
         (std::isdigit(static_cast<unsigned char>(state.peek())) ||
          state.peek() == '.'))
    state.get();
  auto val_opt =
      Utils::FastParseDouble(state.input.substr(start, state.pos - start));
...
NodePtr parse_identifier(ParserState& state) noexcept {
  size_t start = state.pos;
  while (state.has_more() &&
         (std::isalnum(static_cast<unsigned char>(state.peek())) ||
          state.peek() == '_'))
    state.get();
  ...
  if (state.peek() == '(') {
    ...
    state.match(')');
    return state.arena.template alloc<AnyNode>(
        UnaryOpNode(state.arena.allocString(name), arg));
  }

  return state.arena.template alloc<AnyNode>(
      VariableNode(state.arena.allocString(name)));
}
...
NodePtr parse_mul_div(ParserState& state) noexcept {
  ...
  while (state.peek() == '*' || state.peek() == '/') {
    ...
    if (is_matrix_node(left) || is_matrix_node(right)) {
      left = state.arena.template alloc<AnyNode>(
          MatrixBinaryOpNode(op, left, right));
    } else {
...
bool ParseLimitArguments(std::string_view input, std::string_view& expr,
                         std::string_view& var, double& point) noexcept {
...
bool ParseIntegrateArguments(std::string_view input, std::string_view& expr,
...
NodePtr AlgebraicParser::ParseExpression(std::string_view input) noexcept {
...
EngineResult AlgebraicParser::HandleLimit(std::string_view input) noexcept {
  ...
  double point = 0;
  if (!ParseLimitArguments(input, expr, var, point))
    return CreateErrorResult(CalcErr::ParseError);
  SymbolTable ctx;
  ctx.push_back({var, Number(point)});
...
EngineResult AlgebraicParser::HandleIntegrate(std::string_view input) noexcept {
  std::string_view expr, var;
  double a = 0, b = 0;
  if (!ParseIntegrateArguments(input, expr, var, a, b))
    return CreateErrorResult(CalcErr::ParseError);
...
EngineResult AlgebraicParser::ParseAndExecuteWithContext(
    std::string_view input, const SymbolTable& context) noexcept {
  auto root = ParseExpression(input);
  if (!root) return CreateErrorResult(CalcErr::StackOverflow);
  auto res = NodeDispatcher::Evaluate(root, context);
  if (res.HasValue()) {
    if (res.matrix.has_value())
      return CreateSuccessResult(std::move(*res.matrix));
    if (res.vector.has_value())
      return CreateSuccessResult(std::move(*res.vector));
    if (res.value.has_value()) return CreateSuccessResult(*res.value);
  }
  return CreateErrorResult(res.error);
}

EngineResult AlgebraicParser::HandleDerivative(
  ...
  if (target.rfind("derive ", 0) == 0) target = target.substr(7);
  ...
  return CreateSuccessResult(
      NodeDispatcher::ToString(simplified, transient_arena));
}

EngineResult AlgebraicParser::HandleQuadratic(std::string_view input) noexcept {
  return CreateErrorResult(CalcErr::OperationNotFound);
}
EngineResult AlgebraicParser::HandleNonLinearSolve(
    std::string_view input) noexcept {
  return CreateErrorResult(CalcErr::OperationNotFound);
}
EngineResult AlgebraicParser::SolveQuadratic(double a, double b,
                                             double c) noexcept {
  return CreateErrorResult(CalcErr::OperationNotFound);
}
EngineResult AlgebraicParser::SolveNonLinearSystem(
    const FixedVector<std::string_view, 256>& equations,
    SymbolTable& guess) noexcept {
  return CreateErrorResult(CalcErr::OperationNotFound);
}
EngineResult AlgebraicParser::HandlePlotFunction(
    std::string_view input) noexcept {
  return CreateErrorResult(CalcErr::OperationNotFound);
}
