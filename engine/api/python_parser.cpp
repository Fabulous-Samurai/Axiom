...
PythonParser::PythonParser(PythonEngine* engine, PythonMode mode)
    : python_engine_(engine), mode_(mode) {}
...
std::string PythonParser::HandleNumPyMode(std::string_view input) {
  ...
  // Add common NumPy function prefixes - Use FixedVector with explicit
  // constructor
  AXIOM::FixedVector<std::string_view, 256> numpy_funcs(
      {"array", "zeros", "ones", "linspace", "arange", "reshape", "dot",
       "cross", "sum",   "mean", "std",      "min",    "max",     "sqrt",
...
std::string PythonParser::HandleMatplotlibMode(std::string_view input) {
  ...
  AXIOM::FixedVector<std::string_view, 256> plt_funcs(
      {"plot", "scatter", "bar", "hist", "show", "figure", "subplot", "xlabel",
...
std::string PythonParser::HandleSymPyMode(std::string_view input) {
  ...
  AXIOM::FixedVector<std::string_view, 256> sympy_funcs(
      {"Symbol", "symbols", "diff", "integrate", "solve", "expand", "factor",
       "simplify", "limit", "series"});
