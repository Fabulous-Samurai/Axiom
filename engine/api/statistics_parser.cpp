...
AXIOM::FixedVector<double, 256> StatisticsParser::ParseVector(
    const std::string& s) {
  ...
  while (pos < body.size()) {
    ...
    std::string_view token = Utils::Trim(std::string_view(body).substr(
        pos, next == std::string::npos ? std::string_view::npos : next - pos));
    if (!token.empty()) {
...
EngineResult StatisticsParser::ParseAndExecute(
  ...
  if (lower.find("mean") == 0 || lower.find("statsmean") == 0 ||
      lower.find("stats mean") == 0) {
    return engine_->Mean(ParseVector(s));
  }
  if (lower.find("variance") == 0 || lower.find("statsvariance") == 0 ||
      lower.find("stats variance") == 0) {
    return engine_->Variance(ParseVector(s));
  }
  if (lower.find("std") == 0 || lower.find("standarddeviation") == 0 ||
      lower.find("statsstd") == 0) {
