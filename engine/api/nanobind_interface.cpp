...
nb::ndarray<nb::numpy, double, nb::ndim<2>> NanobindInterface::ConvertToNumPyMatrix(
    ...
    return MeasureCall("ConvertToNumPyMatrix", [&]() -> nb::ndarray<nb::numpy, double, nb::ndim<2>> {
        ...
        // ⚡ BOLT OPTIMIZATION:
        // What: Allocate a single 1D vector and reshape it instead of nested vectors.
        // Why: Nested std::vector<std::vector<double>> allocates a new array for every row, which is exceptionally slow.
        // Impact: Reduces row-by-row memory allocations to a single contiguous block, speeding up matrix conversions.
