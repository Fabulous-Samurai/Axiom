...
nb::ndarray<nb::numpy, double, nb::ndim<2>> NanobindInterface::ConvertToNumPyMatrix(
    ...
    return MeasureCall("ConvertToNumPyMatrix", [&]() -> nb::ndarray<nb::numpy, double, nb::ndim<2>> {
        ...
        // ⚡ BOLT OPTIMIZATION:
        // What: Replaced nested std::vector<std::vector<double>> with a flat std::vector<double>.
        // Why: Nested vectors introduce severe memory allocation overhead (O(rows) allocations).
        // Impact: Reduces allocations to a single O(1) allocation, improving performance.
