...
void NanobindInterface::RegisterMethods() { AddToSysPath("."); }
...
std::vector<double> NanobindInterface::ConvertFromNumPy(
    const nb::ndarray<nb::numpy, double>& array) {
  return MeasureCall("ConvertFromNumPy", [&]() -> std::vector<double> {
    ...
    for (size_t i = 0; i < array.size(); ++i) {
      const auto* elem = reinterpret_cast<const double*>(
          base + static_cast<ssize_t>(i) * stride);
      out[i] = *elem;
    }
...
nb::ndarray<nb::numpy, double> NanobindInterface::ConvertToNumPy(
...
AXIOM::EigenEngine::Matrix NanobindInterface::ConvertFromNumPyMatrix(
  ...
  return MeasureCall(
      "ConvertFromNumPyMatrix", [&]() -> AXIOM::EigenEngine::Matrix {
        const size_t rows = array.shape(0);
        const size_t cols = array.shape(1);

        AXIOM::EigenEngine::Matrix matrix(static_cast<Eigen::Index>(rows),
        ...
        for (size_t i = 0; i < rows; ++i) {
          for (size_t j = 0; j < cols; ++j) {
            const auto* elem = reinterpret_cast<const double*>(
                base + static_cast<ssize_t>(i) * s0 +
                static_cast<ssize_t>(j) * s1);
            matrix(static_cast<Eigen::Index>(i), static_cast<Eigen::Index>(j)) =
                *elem;
          }
        }

        UpdateMetrics("ConvertFromNumPyMatrix", 2.0,
                      rows * cols * sizeof(double), false);
        return matrix;
      });
}

nb::ndarray<nb::numpy, double, nb::ndim<2>>
NanobindInterface::ConvertToNumPyMatrix(
  ...
  return MeasureCall(
      ...
      [&]() -> nb::ndarray<nb::numpy, double, nb::ndim<2>> {
        ...
        // ⚡ BOLT OPTIMIZATION:
        // What: Replaced nested std::vector<std::vector<double>> with a flat
        // std::vector<double>. Why: Nested vectors introduce severe memory
        // allocation overhead (O(rows) allocations). Impact: Reduces
        // allocations to a single O(1) allocation, improving performance.
        std::vector<double> flat(rows * cols);
        for (size_t i = 0; i < rows; ++i) {
          for (size_t j = 0; j < cols; ++j) {
            flat[i * cols + j] = matrix(static_cast<Eigen::Index>(i),
                                        static_cast<Eigen::Index>(j));
          }
        }

        auto arr = np.attr("array")(flat, "dtype"_a = "float64")
                       .attr("reshape")(rows, cols);
        UpdateMetrics("ConvertToNumPyMatrix", 1.5, rows * cols * sizeof(double),
                      false);
        return nb::cast<nb::ndarray<nb::numpy, double, nb::ndim<2>>>(arr);
      });
}

AXIOM::EigenEngine::Vector NanobindInterface::ConvertFromNumPyVector(
    const nb::ndarray<nb::numpy, double>& array) {
  return MeasureCall(
      "ConvertFromNumPyVector", [&]() -> AXIOM::EigenEngine::Vector {
        ...
        UpdateMetrics("ConvertFromNumPyVector", 1.2,
                      array.size() * sizeof(double), false);
        return v;
      });
}

nb::ndarray<nb::numpy, double> NanobindInterface::ConvertToNumPyVector(
    const AXIOM::EigenEngine::Vector& vector) {
  return MeasureCall(
      "ConvertToNumPyVector", [&]() -> nb::ndarray<nb::numpy, double> {
...
nb::object NanobindInterface::ExecutePythonCode(const std::string& code) {
  return MeasureCall("ExecutePythonCode", [&]() -> nb::object {
    ...
    PyObject* eval_result = PyRun_StringFlags(
        code.c_str(), Py_eval_input, reinterpret_cast<PyObject*>(globals.ptr()),
        reinterpret_cast<PyObject*>(locals.ptr()), nullptr);
    ...
    PyObject* exec_result = PyRun_StringFlags(
        code.c_str(), Py_file_input, reinterpret_cast<PyObject*>(globals.ptr()),
        reinterpret_cast<PyObject*>(locals.ptr()), nullptr);
...
void NanobindInterface::ClearPythonError() { last_error_.clear(); }

void NanobindInterface::ResetMetrics() { metrics_ = InteropMetrics{}; }
...
size_t NanobindInterface::GetPythonMemoryUsage() const { return 0; }

template <typename Func>
auto NanobindInterface::MeasureCall(const std::string& function_name,
...
void NanobindInterface::UpdateMetrics(const std::string& function_name,
                                      double overhead_us, size_t data_bytes,
                                      bool zero_copy) {
  metrics_.last_function_called = function_name;
...
std::vector<T> FromNumPy(const nb::ndarray<nb::numpy, T>& array) {
  ...
  for (size_t i = 0; i < array.size(); ++i) {
    const auto* elem =
        reinterpret_cast<const T*>(base + static_cast<ssize_t>(i) * stride);
    out[i] = *elem;
  }
  return out;
}
...
template std::vector<double> FromNumPy(
    const nb::ndarray<nb::numpy, double>& array);
template std::vector<float> FromNumPy(
    const nb::ndarray<nb::numpy, float>& array);
template std::vector<int> FromNumPy(const nb::ndarray<nb::numpy, int>& array);

template nb::ndarray<nb::numpy, double> ToNumPy(
    const std::vector<double>& data);
