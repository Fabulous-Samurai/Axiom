...
nb::ndarray<nb::numpy, double, nb::ndim<2>> NanobindInterface::ConvertToNumPyMatrix(
    ...
    return MeasureCall("ConvertToNumPyMatrix", [&]() -> nb::ndarray<nb::numpy, double, nb::ndim<2>> {
        ...
        // ⚡ BOLT OPTIMIZATION:
        // What: Replaced nested std::vector allocations with a single flat vector and reshape.
        // Why: nested std::vector<std::vector<double>> introduces severe memory allocation overhead (O(rows) allocations).
        // Impact: Reduces allocations from O(rows) to O(1), improving Matrix to NumPy array conversion performance.
