...
nb::ndarray<nb::numpy, double, nb::ndim<2>> NanobindInterface::ConvertToNumPyMatrix(
    ...
    return MeasureCall("ConvertToNumPyMatrix", [&]() -> nb::ndarray<nb::numpy, double, nb::ndim<2>> {
        ...
        // ⚡ BOLT OPTIMIZATION:
        // What: Replaced nested std::vector<std::vector<double>> with a flat std::vector<double> allocation.
        // Why: Nested vector allocation creates severe memory overhead (O(rows) allocations).
        // Impact: Reduces allocation overhead, optimizing the critical matrix conversion path.
