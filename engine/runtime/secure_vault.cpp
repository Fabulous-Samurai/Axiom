 ...
 * @brief Implementation of Secure Vault patterns for Phase H (Operation PLUTO
 * EXODUS)
 */
...
bool SecureMantisVault::initialize_enclave() noexcept {
  ...
  // &token, &updated, &eid, NULL); if (ret != SGX_SUCCESS) return false;
#endif
...
bool SecureMantisVault::seal(const void* src, size_t size) noexcept {
  ...
  // This prevents the OS or other processes from reading the Mantis State
  // Machine memory.
  storage_size_ = size;
  ...
  // sgx_seal_data(0, NULL, size, (uint8_t*)src, sealed_len,
  // (sgx_sealed_data_t*)encrypted_storage_.get());
#else
...
bool SecureMantisVault::unseal(void* dst, size_t size) noexcept {
  if (!enclave_active_ || !encrypted_storage_) return false;
    ...
    // sgx_unseal_data((sgx_sealed_data_t*)encrypted_storage_.get(), NULL,
    // &mac_text_len, (uint8_t*)dst, &decrypt_data_len);
#else
...
void SecureMantisVault::secure_transition() noexcept {
  // [ECALL]: Enter Enclave context for high-integrity Mantis heuristic
  // computation. [OCALL]: Call out to non-trusted telemetry scribe if needed.
