...
ProcessProxy::ProcessProxy() { spawn_worker(); }

ProcessProxy::~ProcessProxy() { terminate_worker(); }
...
std::future<SandboxResult> ProcessProxy::execute(
    const std::string& command, std::chrono::milliseconds timeout) {
  return std::async(
      std::launch::async, [this, command, timeout]() -> SandboxResult {
        ...
        SandboxResponse res;
        res.success = true;
        std::snprintf(res.result.data(), res.result.size(), "Result for %s",
                      command.c_str());
        ...
        return {res.success, std::string(res.result.data()),
                std::string(res.error.data()), elapsed};
      });
}

void ProcessProxy::terminate_worker() { is_worker_alive_.store(false); }
