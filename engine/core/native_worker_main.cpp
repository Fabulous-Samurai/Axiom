...
int main() {
    ...
    ipc = (AXIOM::Sandbox::SandboxIPCLayout*)MapViewOfFile(h_map, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(AXIOM::Sandbox::SandboxIPCLayout));
    if (!ipc) {
        CloseHandle(h_map);
        return 1;
    }
    ...
    while (true) {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(100)); // Adaptive polling
    }
