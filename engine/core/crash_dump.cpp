...
bool CrashVault::initialize(const std::string& path) {
    ...
    HANDLE hMapping = CreateFileMappingA(hFile, NULL, PAGE_READWRITE, 0, (DWORD)total_size, NULL);
    if (!hMapping) {
        CloseHandle(hFile);
        return false;
    }
    ...
    mmap_base_ = mmap(NULL, total_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    records_ = reinterpret_cast<CrashRecord*>(static_cast<char*>(mmap_base_) + sizeof(VaultHeader));
    ...
    std::cout << "[AXIOM Phase 3] Crash Vault Initialized at: " << path << " (" << total_size / 1024 << " KB)" << std::endl;
    return true;
}
