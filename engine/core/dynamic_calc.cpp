...
DynamicCalc::DynamicCalc() {
  ...
  parsers_[ModeToIndex(CalculationMode::ALGEBRAIC)]
      .emplace<AXIOM::AlgebraicParser>();
  parsers_[ModeToIndex(CalculationMode::LINEAR_SYSTEM)]
      .emplace<AXIOM::LinearSystemParser>();
  parsers_[ModeToIndex(CalculationMode::STATISTICS)]
      .emplace<AXIOM::StatisticsParser>(statistics_engine_.get());
  parsers_[ModeToIndex(CalculationMode::SYMBOLIC)]
      .emplace<AXIOM::SymbolicParser>(symbolic_engine_.get());
  parsers_[ModeToIndex(CalculationMode::UNITS)].emplace<AXIOM::UnitParser>(
      unit_manager_.get());
...
EngineResult DynamicCalc::Evaluate(std::string_view input) noexcept {
  ...
  if (idx == kInvalidModeIndex)
    return CreateErrorResult(CalcErr::OperationNotFound);

  return std::visit(
      [input](auto& parser) -> EngineResult {
...
EngineResult DynamicCalc::EvaluateFast(double lhs, double rhs,
                                       FastArithmeticOp op) noexcept {
  double out = 0.0;
