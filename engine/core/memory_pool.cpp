 ...
 * @brief Implementation of hardware-pinned memory allocation (Operation
 * DRY-POOL).
 */
...
#ifdef _WIN32
  base_addr_ =
      VirtualAlloc(NULL, size_, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
#else
  base_addr_ = mmap(NULL, size_, PROT_READ | PROT_WRITE,
                    MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (base_addr_ == MAP_FAILED) base_addr_ = nullptr;
#endif
  if (base_addr_) {
    std::cout << "[AXIOM MEMORY] SLM Pool Initialized: " << size_ << " bytes."
              << std::endl;
  }
}
...
bool PinnedMemoryPool::pin() noexcept {
  ...
  if (VirtualLock(base_addr_, size_)) {
    std::cout << "[AXIOM MEMORY] Region pinned to RAM (VirtualLock)."
              << std::endl;
    return true;
  }
#else
...
#endif
  std::cerr << "[AXIOM MEMORY] PINNING FAILURE - Kernel may swap this region!"
            << std::endl;
  return false;
}
