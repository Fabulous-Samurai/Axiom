...
std::string_view FormatNumber(Arena& arena, double val) noexcept {
  ...
  int len = 0;
  if (std::isinf(val))
    len = snprintf(buf, sizeof(buf), std::signbit(val) ? "-inf" : "inf");
  else if (std::isnan(val))
    len = snprintf(buf, sizeof(buf), "nan");
  else if (val == std::floor(val) && std::abs(val) < 1e15)
    len = snprintf(buf, sizeof(buf), "%lld", static_cast<long long>(val));
  else {
    const double abs_val = std::abs(val);
    if (abs_val >= 1e6 || (abs_val > 0 && abs_val < 1e-6))
      len = snprintf(buf, sizeof(buf), "%.6e", val);
    else
      len = snprintf(buf, sizeof(buf), "%.15g", val);
  }
  return arena.allocString(
      std::string_view(buf, len > 0 ? static_cast<size_t>(len) : 0));
}
}  // namespace

EvalResult NodeDispatcher::Evaluate(NodePtr node,
                                    const SymbolTable& vars) noexcept {
  ...
  return std::visit(
      [&](auto&& n) -> EvalResult {
        ...
        if constexpr (std::is_same_v<T, NumberNode>) {
          ...
          auto l_res = Evaluate(n.left, vars);
          if (!l_res.HasValue()) return l_res;
          auto r_res = Evaluate(n.right, vars);
          if (!r_res.HasValue()) return r_res;
          ...
          switch (n.op) {
            ...
            case '*':
              return EvalResult::Success(l * r);
            case '/':
              return (r == 0) ? EvalResult::Failure(CalcErr::DivideByZero)
                              : EvalResult::Success(l / r);
            case '^':
              return EvalResult::Success(std::pow(l, r));
            default:
              return EvalResult::Failure(CalcErr::OperationNotFound);
          }
        } else if constexpr (std::is_same_v<T, UnaryOpNode>) {
          auto res = Evaluate(n.operand, vars);
          ...
          if (n.func == "sqrt")
            return (v < 0) ? EvalResult::Failure(CalcErr::NegativeRoot)
                           : EvalResult::Success(std::sqrt(v));
          if (n.func == "abs") return EvalResult::Success(std::abs(v));
          if (n.func == "log")
            return (v <= 0) ? EvalResult::Failure(CalcErr::DomainError)
                            : EvalResult::Success(std::log10(v));
          ...
          auto r_res = Evaluate(n.right, vars);
          if (!r_res.HasValue()) return r_res;
          if (!l_res.matrix.has_value() || !r_res.matrix.has_value())
            return EvalResult::Failure(CalcErr::ArgumentMismatch);
          const auto& A = *l_res.matrix;
          const auto& B = *r_res.matrix;
          if (n.op == '*') {
...
void NodeDispatcher::CollectVariables(NodePtr node,
                                      SymbolTable& var_map) noexcept {
  if (!node) return;
  std::visit(
      [&](auto&& n) {
        using T = std::decay_t<decltype(n)>;
        if constexpr (std::is_same_v<T, VariableNode>) {
          if (n.name != "pi" && n.name != "e") {
            if (LookupSymbol(var_map, n.name) == -1)
              var_map.push_back({n.name, Number(0.0)});
          }
        } else if constexpr (std::is_same_v<T, BinaryOpNode>) {
...
std::string_view NodeDispatcher::ToString(NodePtr node, Arena& arena,
                                          Precedence p) noexcept {
  if (!node) return "";
  return std::visit(
      [&](auto&& n) -> std::string_view {
        ...
        if constexpr (std::is_same_v<T, NumberNode>) {
          ...
          char buf[1024];
          int len = snprintf(buf, sizeof(buf), "%.*s %c %.*s", (int)l.length(),
                             l.data(), n.op, (int)r.length(), r.data());
          return arena.allocString(
              std::string_view(buf, len > 0 ? static_cast<size_t>(len) : 0));
        } else if constexpr (std::is_same_v<T, UnaryOpNode>) {
          ...
          char buf[1024];
          int len =
              snprintf(buf, sizeof(buf), "%.*s(%.*s)", (int)n.func.length(),
                       n.func.data(), (int)arg.length(), arg.data());
          return arena.allocString(
              std::string_view(buf, len > 0 ? static_cast<size_t>(len) : 0));
        }
...
NodePtr NodeDispatcher::Derivative(NodePtr node, Arena& arena,
  ...
  return std::visit(
      [&](auto&& n) -> NodePtr {
        ...
        if constexpr (std::is_same_v<T, NumberNode>) {
          ...
          return (n.name == var)
                     ? arena.template alloc<AnyNode>(NumberNode(1.0))
                     : arena.template alloc<AnyNode>(NumberNode(0.0));
        } else if constexpr (std::is_same_v<T, BinaryOpNode>) {
          auto dl = Derivative(n.left, arena, var);
          auto dr = Derivative(n.right, arena, var);
          if (n.op == '+' || n.op == '-')
            return arena.template alloc<AnyNode>(BinaryOpNode(n.op, dl, dr));
          if (n.op == '*') {
            auto term1 =
                arena.template alloc<AnyNode>(BinaryOpNode('*', dl, n.right));
            auto term2 =
                arena.template alloc<AnyNode>(BinaryOpNode('*', n.left, dr));
            return arena.template alloc<AnyNode>(
                BinaryOpNode('+', term1, term2));
          }
          return arena.template alloc<AnyNode>(NumberNode(0.0));
        }
        return arena.template alloc<AnyNode>(NumberNode(0.0));
  ...
  return std::visit(
      [&](auto&& n) -> NodePtr {
        ...
        if constexpr (std::is_same_v<T, BinaryOpNode>) {
          ...
          return arena.template alloc<AnyNode>(
              UnaryOpNode(n.func, Simplify(n.operand, arena)));
        }
        return node;
      },
      *node);
