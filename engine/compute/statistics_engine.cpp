...
EngineResult StatisticsEngine::Median(Vector data) {
    ...
    if (n % 2 == 0) {
        ...
        return CreateSuccessResult((*max_left + *mid) / 2.0);
    } else {
...
EngineResult StatisticsEngine::Percentile(Vector data, double p) {
    ...
    if (p == 0) {
        return CreateSuccessResult(*std::min_element(data.begin(), data.end()));
    }
    if (p == 100) {
    ...
    if (upper >= data.size()) {
        ...
        return CreateSuccessResult(data[lower]);
    }
    
    auto upper_it = data.begin() + upper;
    std::nth_element(data.begin(), upper_it, data.end());

    auto lower_it = std::max_element(data.begin(), upper_it);

    double weight = index - lower;
    double result = (*lower_it) * (1.0 - weight) + (*upper_it) * weight;
