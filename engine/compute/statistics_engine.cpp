...
EngineResult StatisticsEngine::Variance(const Vector& data) {
  ...
  // Use the EngineResult helper to extract a double regardless of underlying
  // variant
  auto mean_val_opt = mean_result.GetDouble();
  if (!mean_val_opt.has_value())
...
EngineResult StatisticsEngine::LinearRegression(const Vector& x,
                                                const Vector& y) {
  if (x.size() != y.size() || x.size() < 2) {
    return CreateErrorResult(CalcErr::ArgumentMismatch);
  }
...
EngineResult StatisticsEngine::MovingAverage(const Vector& data,
                                             int window_size) {
  if (data.empty() || window_size <= 0 ||
      window_size > static_cast<int>(data.size())) {
    return CreateErrorResult(CalcErr::ArgumentMismatch);
  }
