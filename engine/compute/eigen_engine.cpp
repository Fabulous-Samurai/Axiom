...
EigenEngine::EigenEngine() {
  ...
  std::cout << "EigenEngine initialized with Senna Speed optimization!"
            << std::endl;
  std::cout << "   Threads: " << num_threads_ << std::endl;
  std::cout << "   SIMD: " << (simd_enabled_ ? "Enabled" : "Disabled")
            << std::endl;
}
...
EigenEngine::Matrix EigenEngine::CreateMatrix(
    const AXIOM::FixedVector<AXIOM::FixedVector<double, 256>, 256>& data)
    const {
...
EigenEngine::Vector EigenEngine::CreateVector(
    const AXIOM::FixedVector<double, 256>& data) const {
...
EigenEngine::Matrix EigenEngine::MatrixMultiply(const Matrix& A,
...
EigenEngine::Vector EigenEngine::MatrixVectorMultiply(const Matrix& A,
...
EigenEngine::Matrix EigenEngine::MatrixAdd(const Matrix& A,
                                           const Matrix& B) const {
  SENNA_SPEED_EIGEN("MatrixAdd");
  return A + B;
}

EigenEngine::Matrix EigenEngine::MatrixSubtract(const Matrix& A,
                                                const Matrix& B) const {
...
std::optional<EigenEngine::Matrix> EigenEngine::Inverse(
    const Matrix& A) const noexcept {
...
std::optional<std::pair<EigenEngine::Vector, EigenEngine::Matrix>>
EigenEngine::EigenDecomposition(const Matrix& A) const noexcept {
  ...
  {  // try removed
    ...
    if (A.isApprox(A.transpose())) {
      ...
      return std::make_pair(solver.eigenvalues().real(),
...
std::tuple<EigenEngine::Matrix, EigenEngine::Vector, EigenEngine::Matrix>
EigenEngine::SVD(const Matrix& A) const {
...
EigenEngine::Vector EigenEngine::SolveLinearSystem(const Matrix& A,
...
EigenEngine::Matrix EigenEngine::SolveMultipleRHS(const Matrix& A,
                                                  const Matrix& B) const {
...
EigenEngine::Vector EigenEngine::Convolution(const Vector& signal1,
                                             const Vector& signal2) const {
  ...
  for (int i = 0; i < signal1.size(); ++i)
    a(i) = std::complex<double>(signal1(i), 0.0);
  for (int i = 0; i < signal2.size(); ++i)
    b(i) = std::complex<double>(signal2(i), 0.0);
...
EigenEngine::Vector EigenEngine::CrossCorrelation(const Vector& signal1,
                                                  const Vector& signal2) const {
...
std::string EigenEngine::MatrixToString(const Matrix& mat,
                                        int precision) const {
  std::stringstream ss;
...
std::string EigenEngine::VectorToString(const Vector& vec,
                                        int precision) const {
  std::stringstream ss;
  ss << std::fixed << std::setprecision(precision);
...
std::string EigenEngine::GetPerformanceReport() const {
  ...
  if (last_metrics_.execution_time_ms < 1.0) {
    ...
    ss << "\n   🐌 Needs Optimization (>" << last_metrics_.execution_time_ms
       << "ms)";
  }
...
EigenEngine::Matrix EigenEngine::OptimizedMatMul(const Matrix& A,
                                                 const Matrix& B) const {
...
EigenEngine::Vector EigenEngine::OptimizedMatVecMul(const Matrix& A,
                                                    const Vector& x) const {
  Vector result = A * x;
...
auto EigenEngine::MeasurePerformance(
    Func&& func, const std::string& operation) const -> decltype(func()) {
  ...
  // THREAD-SAFETY FIX: No const_cast needed - UpdateMetrics uses mutable
  // members
...
void EigenEngine::UpdateMetrics(const std::string& operation, double time_ms,
