...
template <size_t N>
void AppendToBuffer(AXIOM::FixedVector<char, N>& buffer,
...
template <size_t N>
void NumberToBuffer(double value,
                    AXIOM::FixedVector<char, N>& buffer) noexcept {
  char temp[64];
  auto [ptr, ec] = std::to_chars(temp, temp + sizeof(temp), value,
                                 std::chars_format::fixed, 12);
  if (ec == std::errc{}) {
...
std::string_view ReplaceVariableTokens(Arena& arena, std::string_view expr,
                                       std::string_view var,
                                       std::string_view replacement) noexcept {
  AXIOM::FixedVector<char, 4096> out;
  for (size_t i = 0; i < expr.size();) {
    bool starts_token = (i == 0 || !IsIdentifierChar(expr[i - 1]));
    if (starts_token && i + var.size() <= expr.size() &&
        expr.substr(i, var.size()) == var) {
      const size_t right = i + var.size();
      const bool ends_token =
          (right == expr.size() || !IsIdentifierChar(expr[right]));
      if (ends_token) {
...
EngineResult EvalScalar(std::string_view expr,
                        const AXIOM::SymbolTable& context = {}) noexcept {
...
bool EvalDouble(std::string_view expr, double& out,
                const AXIOM::SymbolTable& context = {}) noexcept {
...
bool BisectionRoot(std::string_view expr, std::string_view var, double left,
                   double right, double& root) noexcept {
...
EngineResult SymbolicEngine::Expand(std::string_view expression) noexcept {
  ...
  if (expr.size() >= 6 && expr.front() == '(' &&
      expr.find(')') != std::string_view::npos && expr.back() >= '0' &&
      expr.back() <= '9') {
    if (const size_t close = expr.find(')');
        close + 2 < expr.size() && expr[close + 1] == '^') {
      ...
      if (ParseDoubleStrict(power_s, power_d) && IsIntegerValue(power_d)) {
        if (const int n = static_cast<int>(std::round(power_d));
            n >= 0 && n <= 16) {
...
EngineResult SymbolicEngine::Factor(std::string_view expression) noexcept {
  ...
  if (size_t x2 = candidate.find("x^2"); x2 != std::string_view::npos) {
    ...
    if (size_t x_pos = tail.find('x'); x_pos != std::string_view::npos) {
      ...
      if (std::string_view factored = FactorQuadratic(b_str, c_str);
          !factored.empty()) {
...
EngineResult SymbolicEngine::Substitute(std::string_view expr,
                                        std::string_view var,
  ...
  const std::string_view replaced = ReplaceVariableTokens(
      arena_, expr, trimmed_var,
      std::string_view(replacement.data(), replacement.size()));
  return CreateSuccessResult(replaced);
}

EngineResult SymbolicEngine::Integrate(std::string_view expression,
                                       std::string_view variable) noexcept {
  arena_.reset();
  ...
  if (ParseDoubleStrict(expr, c)) {
    ...
    return CreateSuccessResult(
        arena_.allocString(std::string_view(res_buf.data(), res_buf.size())));
  }
  ...
  if (expr.size() == var.size() + 5 && expr.starts_with("sin(") &&
      expr.ends_with(")") && expr.substr(4, var.size()) == var) {
    AXIOM::FixedVector<char, 128> res_buf;
  ...
  if (expr.size() == var.size() + 5 && expr.starts_with("cos(") &&
      expr.ends_with(")") && expr.substr(4, var.size()) == var) {
    AXIOM::FixedVector<char, 128> res_buf;
  ...
  if (expr.starts_with(var) && expr.size() > var.size() &&
      expr[var.size()] == '^') {
    double n = 0.0;
    if (ParseDoubleStrict(expr.substr(var.size() + 1), n) &&
        std::abs(n + 1.0) > 1e-12) {
...
EngineResult SymbolicEngine::DefiniteIntegral(std::string_view expr,
                                              std::string_view var, double a,
                                              double b) noexcept {
  arena_.reset();
  ...
  EngineResult res =
      EvalScalar(std::string_view(command.data(), command.size()));
  if (res.HasResult()) {
    return res;
  }
...
EngineResult SymbolicEngine::PartialDerivative(std::string_view expr,
  ...
  EngineResult res =
      parser.ParseAndExecute(std::string_view(cmd_buf.data(), cmd_buf.size()));
  if (!res.result.has_value()) {
    return CreateErrorResult(CalcErr::OperationNotFound);
  }

  const std::string_view* deriv_sv =
      std::get_if<std::string_view>(&*res.result);
  if (deriv_sv == nullptr) {
    return CreateErrorResult(CalcErr::OperationNotFound);
  }
  ...
  if (variable != "x") {
    return CreateSuccessResult(
        ReplaceVariableTokens(arena_, *deriv_sv, "x", variable));
  }
  return CreateSuccessResult(*deriv_sv);
}

EngineResult SymbolicEngine::TaylorSeries(std::string_view expr,
                                          std::string_view var, double point,
                                          int order) noexcept {
  arena_.reset();
  ...
  for (int k = 0; k <= order; ++k) {
    CalculateTaylorTerm(series, variable, point, k, fact, current_ast,
                        any_term);

    if (k < order) {
      current_ast =
          NodeDispatcher::Derivative(current_ast, parser.GetArena(), variable);
  ...
  return CreateSuccessResult(
      arena_.allocString(std::string_view(series.data(), series.size())));
}

EngineResult SymbolicEngine::SolveEquation(std::string_view equation,
                                           std::string_view variable) noexcept {
  arena_.reset();
...
EngineResult SymbolicEngine::SolveSystem(
    const AXIOM::FixedVector<std::string_view, 256>& equations,
    const AXIOM::FixedVector<std::string_view, 256>& variables) noexcept {
  arena_.reset();
  ...
  AXIOM::AlgebraicParser parser;
  EngineResult res =
      parser.ParseAndExecute(std::string_view(command.data(), command.size()));
  if (res.HasResult()) {
    return res;
  }
  return CreateErrorResult(CalcErr::OperationNotFound);
...
EngineResult SymbolicEngine::FindLimits(std::string_view expr,
                                        std::string_view var,
                                        double approach_point) noexcept {
  arena_.reset();
  if (IsBlank(expr) || IsBlank(var)) {
    return CreateErrorResult(CalcErr::ArgumentMismatch);
  }
...
EngineResult SymbolicEngine::FindRoots(std::string_view expr,
                                       std::string_view var, double range_min,
...
std::string_view SymbolicEngine::ExpandBinomial(std::string_view var_name,
                                                double b_num, int n) noexcept {
...
std::string_view SymbolicEngine::FactorQuadratic(
    std::string_view b_str, std::string_view c_str) noexcept {
  double b = 0.0, c = 0.0;
  if (ParseDoubleStrict(b_str, b) && ParseDoubleStrict(c_str, c) &&
      IsIntegerValue(b) && IsIntegerValue(c)) {
...
bool SymbolicEngine::CalculateTaylorTerm(FixedVector<char, 2048>& series,
                                         ...
                                         double point, int k, double fact,
                                         NodePtr ast, bool& any_term) noexcept {
  SymbolTable context;
...
void SymbolicEngine::BuildEquationExpression(
    AXIOM::FixedVector<char, 2048>& buffer, std::string_view eq) noexcept {
  if (const size_t equal_pos = eq.find('=');
