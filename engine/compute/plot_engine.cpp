 ...
 */
template <size_t N>
static void AppendChars(FixedVector<char, N>& buffer,
 ...
 */
template <size_t N>
static void FormatChars(FixedVector<char, N>& buffer, const char* format,
                        ...) noexcept {
  char temp[128];
  va_list args;
...
char PlotEngine::GetCharForValue(double value, double min_val,
  ...
  const int level =
      static_cast<int>((value - min_val) / range * (num_levels - 1));
  return intensity[std::clamp(level, 0, num_levels - 1)];
}

std::pair<int, int> PlotEngine::MapToScreen(double x, double y,
  ...
  const int screen_x =
      static_cast<int>((x - config.x_min) / safe_x_range * (w - 1));
  const int screen_y =
      static_cast<int>((config.y_max - y) / safe_y_range * (h - 1));

  return {screen_x, screen_y};
}

std::string_view PlotEngine::PlotFunction(std::string_view expression,
                                          const PlotConfig& config) noexcept {
  arena_.reset();
  ...
  const double x_step =
      (std::abs(x_range) > 1e-12) ? (x_range / (width - 1)) : 0.0;
    ...
    if (0 >= x_min && 0 <= x_max) {
      ...
      if (axis_x >= 0 && axis_x < width) {
        for (int row = 0; row < height; ++row) {
          if (grid[row * width + axis_x] == ' ')
            grid[row * width + axis_x] = '|';
        }
      }
    }
    if (0 >= y_min && 0 <= y_max) {
      ...
      if (axis_y >= 0 && axis_y < height) {
        for (int col = 0; col < width; ++col) {
          if (grid[axis_y * width + col] == ' ')
            grid[axis_y * width + col] = '-';
        }
      }
    }
  }
  ...
  return arena_.allocString(
      std::string_view(result_buffer.data(), result_buffer.size()));
}

std::string_view PlotEngine::PlotParametric(std::string_view x_expr,
                                            std::string_view y_expr,
  ...
  const double t_step =
      (std::abs(t_range) > 1e-12) ? (t_range / (samples - 1)) : 0.0;
  ...
  for (int i = 0; i < samples; ++i) {
    ...
    if (const auto x_opt = res_x.GetDouble(); x_opt) {
      if (const auto y_opt = res_y.GetDouble(); y_opt) {
        ...
        const double y = *y_opt;
        if (std::isfinite(x) && std::isfinite(y) && x >= config.x_min &&
            x <= config.x_max && y >= config.y_min && y <= config.y_max) {
          const auto [screen_x, screen_y] = MapToScreen(x, y, config);
          if (screen_x >= 0 && screen_x < width && screen_y >= 0 &&
              screen_y < height) {
            grid[screen_y * width + screen_x] = config.plot_char;
  ...
  if (config.show_axes) {
    ...
    if (ax >= 0 && ax < width)
      for (int r = 0; r < height; ++r)
        if (grid[r * width + ax] == ' ') grid[r * width + ax] = '|';
    if (ay >= 0 && ay < height)
      for (int c = 0; c < width; ++c)
        if (grid[ay * width + c] == ' ') grid[ay * width + c] = '-';
  }
...
Matrix PlotEngine::ComputeFunctionData(std::string_view expression,
                                       const PlotConfig& config,
                                       int samples) noexcept {
  AlgebraicParser parser;
  Matrix data;
  const int n = (samples > 0) ? std::min(samples, 256)
                              : std::max(2, std::min(config.width, 256));
  const double x_range = config.x_max - config.x_min;
...
std::string_view PlotEngine::PlotData(const Vector& x_data,
                                      const Vector& y_data,
                                      const PlotConfig& config) noexcept {
  arena_.reset();
  if (x_data.size() != y_data.size() || x_data.empty()) {
    return arena_.allocString(
        "Error: Data vectors must be same size and non-empty\n");
  }
...
std::string_view PlotEngine::Histogram(const Vector& data, int bins,
                                       const PlotConfig& config) noexcept {
  arena_.reset();
  ...
  for (int i = actual_bins - 1; i >= 0; --i) {
    ...
    const int bar_length = static_cast<int>(
        (static_cast<double>(hist[i]) * config.width) / max_freq);

    FormatChars(result_buffer, "[%7.2f - %7.2f) ", bin_start, bin_end);
    for (int j = 0; j < bar_length; ++j)
      result_buffer.push_back(config.plot_char);
...
std::string_view PlotEngine::BoxPlot(const Vector& data,
                                     const PlotConfig& config) noexcept {
  arena_.reset();
  ...
  FixedVector<char, 1024> line1;
  line1.assign(width, ' ');
  line1[map_x(min_val)] = '|';
  line1[map_x(max_val)] = '|';
  for (int i = map_x(min_val) + 1; i < map_x(max_val); ++i) line1[i] = '-';

  FixedVector<char, 1024> line2;
  line2.assign(width, ' ');
  line2[map_x(q1)] = '[';
  line2[map_x(q3)] = ']';
  line2[map_x(median)] = '|';
  for (int i = map_x(q1) + 1; i < map_x(q3); ++i)
    if (i != map_x(median)) line2[i] = '=';

  FixedVector<char, 4096> result;
  AppendChars(result, "Box Plot:\n");
  AppendChars(result, std::string_view(line1.data(), line1.size()));
  AppendChars(result, "\n");
  AppendChars(result, std::string_view(line2.data(), line2.size()));
  AppendChars(result, "\n");
  AppendChars(result, std::string_view(line1.data(), line1.size()));
  AppendChars(result, "\n");
  FormatChars(result,
...
std::string_view PlotEngine::PlotSurface(std::string_view expression,
                                         const PlotConfig& config) noexcept {
  arena_.reset();
  AlgebraicParser parser;
  const int width = std::clamp(config.width, 1, 256);
  ...
  const double x_max = config.x_max;
  const double y_min =
      config.x_min;  // Use symmetric domain for y input if not specified
  ...
  for (int r = 0; r < height; ++r) {
    for (int c = 0; c < width; ++c) {
      ...
      if (const auto z_opt = res.GetDouble(); z_opt) {
        grid[r * width + c] =
            GetCharForValue(*z_opt, config.y_min, config.y_max);
      } else {
        grid[r * width + c] = ' ';
      }
    }
...
std::string_view PlotEngine::PlotComplex(const Vector& real_parts,
...
std::string_view PlotEngine::PolarPlot(std::string_view r_expression,
                                       const PlotConfig& config) noexcept {
  arena_.reset();
  AlgebraicParser parser;
  const int width = std::clamp(config.width, 1, 256);
