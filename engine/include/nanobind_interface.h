...
class NanobindInterface {
 public:
  ...
  std::vector<double> ConvertFromNumPy(
      const nb::ndarray<nb::numpy, double>& array);
  nb::ndarray<nb::numpy, double> ConvertToNumPy(
      const std::vector<double>& data);

#ifdef ENABLE_EIGEN
  // Eigen integration with zero-copy
  AXIOM::EigenEngine::Matrix ConvertFromNumPyMatrix(
      const nb::ndarray<nb::numpy, double, nb::ndim<2>>& array);
  nb::ndarray<nb::numpy, double, nb::ndim<2>> ConvertToNumPyMatrix(
      const AXIOM::EigenEngine::Matrix& matrix);

  AXIOM::EigenEngine::Vector ConvertFromNumPyVector(
      const nb::ndarray<nb::numpy, double>& array);
  nb::ndarray<nb::numpy, double> ConvertToNumPyVector(
 ...
 private:
  ...
  auto MeasureCall(const std::string& function_name, Func&& func)
      -> decltype(func());
...
#ifdef ENABLE_EIGEN
// Eigen-NumPy bridge
AXIOM::EigenEngine::Matrix MatrixFromNumPy(
    const nb::ndarray<nb::numpy, double, nb::ndim<2>>& array);
nb::ndarray<nb::numpy, double, nb::ndim<2>> MatrixToNumPy(
    const AXIOM::EigenEngine::Matrix& matrix);
#endif
