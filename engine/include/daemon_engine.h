...
class DaemonEngine {
 ...
 private:
  ...
  AXIOM::FixedVector<
      std::pair<std::string_view, std::unique_ptr<struct SessionContext>>, 128>
      sessions_;
 ...
 public:
  ...
  [[nodiscard]] bool is_running() const noexcept {
    return running_.load(std::memory_order::seq_cst);
  }
  [[nodiscard]] DaemonStatus get_status() const noexcept {
    return status_.load(std::memory_order::seq_cst);
  }

  [[nodiscard]] Response process_request(const Request& request) noexcept;
  [[nodiscard]] bool send_command(std::string_view session_id,
                                  std::string_view command,
                                  std::string_view mode = "algebraic") noexcept;

  [[nodiscard]] std::expected<std::string_view, std::error_code>
  create_session() noexcept;
  ...
  [[nodiscard]] uint64_t get_total_requests() const noexcept {
    return total_requests_.load(std::memory_order::seq_cst);
  }
  [[nodiscard]] uint64_t get_rejected_requests() const noexcept {
    return rejected_requests_.load(std::memory_order::seq_cst);
  }
  [[nodiscard]] double get_avg_response_time() const noexcept {
    return avg_response_time_.load(std::memory_order::seq_cst);
  }
