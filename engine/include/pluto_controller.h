...
struct AXIOM_EXPORT PlutoState {
  ...
  bool operator==(const PlutoState& other) const noexcept {
    return p1_queue.load(std::memory_order_relaxed) ==
               other.p1_queue.load(std::memory_order_relaxed) &&
           p2_active.load(std::memory_order_relaxed) ==
               other.p2_active.load(std::memory_order_relaxed) &&
           p3_done.load(std::memory_order_relaxed) ==
               other.p3_done.load(std::memory_order_relaxed) &&
           p4_available.load(std::memory_order_relaxed) ==
               other.p4_available.load(std::memory_order_relaxed);
  }
};
...
class AXIOM_EXPORT PlutoController {
 public:
   ...
   */
  float get_scaling_factor() const noexcept {
    return solver_.get_scaling_factor();
  }

  /**
   * @brief Get the current search tree nodes for streaming to GUI.
   */
  size_t get_search_tree(Mantis::AStarNode* out_nodes,
                         size_t max_count) noexcept;

  /**
   * @brief Map a Petri Net state to a Mantis feature vector.
   */
  static Mantis::NodeFeatureVecF32 state_to_features(
      const PlutoState& state) noexcept {
    Mantis::NodeFeatureVecF32 f;
    f.data[0] =
        static_cast<float>(state.p1_queue.load(std::memory_order_relaxed));
    f.data[1] =
        static_cast<float>(state.p2_active.load(std::memory_order_relaxed));
    f.data[2] =
        static_cast<float>(state.p3_done.load(std::memory_order_relaxed));
    f.data[3] =
        static_cast<float>(state.p4_available.load(std::memory_order_relaxed));
