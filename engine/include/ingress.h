 ...
 *
 * Replaces dynamic polymorphism with static-variant dispatch for sub-nanosecond
 * ingress.
 */
 ...
 */
using AnyIngressChannel =
    std::variant<AF_XDPChannel, PosixUDPChannel, MockHFTChannel>;
...
struct IngressDispatcher {
  static size_t poll(
      AnyIngressChannel& channel,
      std::function<void(const IngressFrame&)> callback) noexcept {
    return std::visit(
        [&](auto& concrete_channel) { return concrete_channel.poll(callback); },
        channel);
  }

  static bool start(AnyIngressChannel& channel,
                    HarmonicArena* target_arena) noexcept {
    return std::visit(
  ...
  static void stop(AnyIngressChannel& channel) noexcept {
    std::visit([&](auto& concrete_channel) { concrete_channel.stop(); },
               channel);
  }
};
...
class AXIOM_EXPORT IngressFactory {
 public:
  enum class ProviderType { PROV_AF_XDP, PROV_POSIX_UDP, PROV_MOCK_HFT_FEED };

  static AnyIngressChannel create(ProviderType type,
