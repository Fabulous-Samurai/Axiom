 ...
 */
using AnyNode =
    std::variant<NumberNode, VariableNode, BinaryOpNode, UnaryOpNode,
                 MultiArgFunctionNode, MatrixNode, MatrixBinaryOpNode>;
...
struct EvalResult {
  ...
  static EvalResult Success(double val) noexcept {
    EvalResult r;
    r.value = Number(val);
    return r;
  }
  static EvalResult Success(const Number& val) noexcept {
    EvalResult r;
    r.value = val;
    return r;
  }
  static EvalResult Success(Vector&& val) noexcept {
    EvalResult r;
    r.vector = std::move(val);
    return r;
  }
  static EvalResult Success(Matrix&& val) noexcept {
    EvalResult r;
    r.matrix = std::move(val);
    return r;
  }
  static EvalResult Failure(CalcErr err) noexcept {
    EvalResult r;
    r.error = err;
    return r;
  }
  bool HasValue() const noexcept {
    return (value || vector || matrix) && error == CalcErr::None;
  }
};
...
struct alignas(64) BinaryOpNode {
  ...
  NodePtr left;
  NodePtr right;
  constexpr BinaryOpNode(char o, NodePtr l, NodePtr r) noexcept
      : op(o), left(l), right(r) {}
};
...
struct alignas(64) UnaryOpNode {
  ...
  NodePtr operand;
  constexpr UnaryOpNode(std::string_view f, NodePtr op) noexcept
      : func(f), operand(op) {}
};
...
struct alignas(64) MultiArgFunctionNode {
  ...
  MultiArgFunctionNode(std::string_view f, FixedVector<NodePtr, 16> a) noexcept
...
struct alignas(64) MatrixBinaryOpNode {
  ...
  NodePtr left;
  NodePtr right;
  constexpr MatrixBinaryOpNode(char o, NodePtr l, NodePtr r) noexcept
      : op(o), left(l), right(r) {}
};
...
struct NodeDispatcher {
  static EvalResult Evaluate(const AnyNode* node,
                             const SymbolTable& vars) noexcept;
  static std::string_view ToString(const AnyNode* node, Arena& arena,
                                   Precedence p = Precedence::None) noexcept;
  static void CollectVariables(const AnyNode* node,
                               SymbolTable& var_map) noexcept;
  static NodePtr Derivative(const AnyNode* node, Arena& arena,
                            std::string_view var) noexcept;
  static NodePtr Simplify(const AnyNode* node, Arena& arena) noexcept;

#if defined(ASMJIT_BUILD_X86)
  static bool CompileX86(const AnyNode* node, asmjit::x86::Compiler& cc,
                         asmjit::x86::Gp vars_ptr, const SymbolTable& var_map,
