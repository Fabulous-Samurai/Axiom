...
inline ExpressionPolicyDecision AssessExpressionPolicy(
    std::string_view input, CalculationMode mode) noexcept {
  ...
  const std::size_t max_chars = (mode == CalculationMode::SYMBOLIC)
                                    ? kMaxCharsSymbolic
                                    : kMaxCharsDefault;
  const std::size_t max_depth = (mode == CalculationMode::SYMBOLIC)
                                    ? kMaxDepthSymbolic
                                    : kMaxDepthDefault;
  ...
  for (char ch : input) {
    ...
    if (ch == '(' || ch == '[' || ch == '{') {
      ++current_depth;
      max_observed_depth = (max_observed_depth > current_depth)
                               ? max_observed_depth
                               : current_depth;
    } else if (ch == ')' || ch == ']' || ch == '}') {
      if (current_depth == 0) {
  ...
  if (approx_matrix_elements > 0 &&
      (approx_matrix_elements + 1) > kMaxMatrixElements) {
