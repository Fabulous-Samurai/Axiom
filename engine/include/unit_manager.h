...
class AXIOM_EXPORT UnitManager {
 ...
 public:
  ...
  // Core functionality
  EngineResult ConvertUnit(double value, std::string_view from_unit,
                           std::string_view to_unit) noexcept;
  EngineResult ConvertTemperature(double value, std::string_view from_unit,
                                  std::string_view to_unit) noexcept;
  ...
  // Registration
  void RegisterUnit(std::string_view symbol, UnitType type, double scale,
                    std::string_view name) noexcept;
  AXIOM::FixedVector<std::string_view, 256> GetUnitsOfType(
