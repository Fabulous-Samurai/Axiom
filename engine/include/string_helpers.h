...
inline std::optional<double> FastParseDouble(std::string_view sv) {
  ...
  // Why: std::string construction involves dynamic allocation and copying which
  // is costly in loops. We only need it for edge cases (.5 or 5.) Impact:
  // Reduces time in FastParseDouble by ~13% (360ms to 312ms on 10M iterations).
