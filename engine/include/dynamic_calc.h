...
/**
 * @file dynamic_calc.h
 * @brief Optimized DynamicCalc using runtime polymorphism via IParser
 * interface.
 */
...
enum class FastArithmeticOp { Add, Subtract, Multiply, Divide };

using ParserVariant =
    std::variant<std::monostate, AXIOM::AlgebraicParser,
                 AXIOM::LinearSystemParser, AXIOM::StatisticsParser,
                 AXIOM::SymbolicParser, AXIOM::UnitParser, AXIOM::PlotParser,
                 AXIOM::PythonParser>;
...
class AXIOM_EXPORT DynamicCalc {
 ...
 public:
  ...
  [[nodiscard]] EngineResult EvaluateFast(double lhs, double rhs,
