...
class AXIOM_EXPORT AlgebraicParser : public IParser {
 public:
  ...
  EngineResult ParseAndExecuteWithContext(std::string_view input,
                                          const SymbolTable& context) noexcept;
 ...
 private:
  ...
  mutable FixedVector<std::pair<std::string_view, EvalResult>, 1024>
      eval_cache_;
  ...
  EngineResult SolveNonLinearSystem(
