...
class SymbolicExpression {
 ...
 public:
  ...
  SymbolicExpression integrate(const std::string& variable, double lower_bound,
                               double upper_bound) const;
  SymbolicExpression limit(const std::string& variable,
                           const SymbolicExpression& value,
                           const std::string& direction = "") const;
  SymbolicExpression series(const std::string& variable,
                            const SymbolicExpression& point, int order) const;
  ...
  SymbolicExpression substitute(const std::string& variable,
                                const SymbolicExpression& value) const;
  SymbolicExpression substitute(
      const std::unordered_map<std::string, SymbolicExpression>& substitutions)
      const;

  // Solving
  std::vector<SymbolicExpression> solve(const std::string& variable) const;
  std::vector<SymbolicExpression> solve(
  ...
  std::complex<double> evaluate_complex(
      const std::unordered_map<std::string, std::complex<double>>& values)
      const;
  ...
  std::vector<SymbolicExpression> get_coefficients(
...
class SymbolManager {
 private:
  ...
  std::unordered_map<std::string, std::string>
      symbol_properties_;  // real, positive, integer, etc.
 ...
 public:
  ...
  void define_symbol_property(const std::string& name,
...
class SymbolicMatrix {
 ...
 public:
  ...
  SymbolicMatrix(const std::vector<SymbolicExpression>& data, size_t rows,
                 size_t cols);
  ...
  static SymbolicMatrix diagonal(
      const std::vector<SymbolicExpression>& diag_elements);
...
class SymbolicODESolver {
 ...
 public:
  ...
  ODESolution solve_ivp(
      const SymbolicExpression& equation, const std::string& dependent_var,
      const std::string& independent_var,
      const std::unordered_map<std::string, double>& initial_conditions) const;

  // Solve systems of ODEs
  std::vector<ODESolution> solve_system(
...
class ComputerAlgebraSystem {
 ...
 public:
  ...
  SymbolicExpression create_function(
      const std::string& name, const std::vector<SymbolicExpression>& args);
  ...
  SymbolicExpression diff(const std::string& expression,
                          const std::string& variable, int order = 1);
  SymbolicExpression integrate(const std::string& expression,
                               const std::string& variable);
  SymbolicExpression integrate(const std::string& expression,
                               const std::string& variable, double lower,
                               double upper);

  SymbolicExpression limit(const std::string& expression,
                           const std::string& variable,
                           const std::string& value,
                           const std::string& direction = "");
  SymbolicExpression series(const std::string& expression,
                            const std::string& variable,
                            const std::string& point, int order);

  std::vector<SymbolicExpression> solve(const std::string& equation,
                                        const std::string& variable);
  std::vector<SymbolicExpression> solve(
  ...
  // Utility functions
  std::string simplify_and_format(const std::string& expression,
                                  const std::string& format = "default");
  double evaluate_numeric(const std::string& expression);
  double evaluate_numeric(
...
class SymEngineOptimizer {
 ...
 public:
  ...
  std::vector<SymbolicExpression> optimize_expressions(
      const std::vector<SymbolicExpression>& exprs);
 ...
 private:
  ...
  void cache_expression(const std::string& hash,
                        const SymbolicExpression& expr);
};

// Convenience functions for common symbolic operations
namespace SymbolicFunctions {
SymbolicExpression diff(const SymbolicExpression& expr, const std::string& var,
                        int order = 1);
SymbolicExpression integrate(const SymbolicExpression& expr,
...
SymbolicExpression pow(const SymbolicExpression& base,
