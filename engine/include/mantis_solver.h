...
// ============================================================================
inline constexpr size_t kMaxNodes =
    16384;  // Fixed-capacity for zero-allocation
...
class AXIOM_EXPORT IDAStarSolver {
 public:
  ...
  AXIOM_FORCE_INLINE float Heuristic(const AStarNode& node) const noexcept {
    float raw_h = MantisHeuristic::evaluate_f32(node.features, target_profile_,
                                                dog_threshold_);
  ...
  SearchResult solve(AStarNode* nodes, uint32_t num_nodes, uint32_t start_id,
                     uint32_t goal_id, AdjacencyFn&& get_neighbors) noexcept {
    if (num_nodes == 0 || num_nodes > kMaxNodes || start_id >= num_nodes)
        [[unlikely]] {
      return {};
    }
    ...
    // Use a persistent neighbor buffer to avoid thread_local overhead in
    // recursion
    ...
    while (true) {
      ...
      bool found =
          search(nodes, start_id, goal_id, 0.0f, threshold, next_threshold,
                 result, get_neighbors, num_nodes, neighbor_buf);
 ...
 private:
   ...
   */
  template <typename AdjacencyFn>
  bool search(AStarNode* nodes, uint32_t current_id, uint32_t goal_id,
              float g_cost, float threshold, float& next_threshold,
              SearchResult& result, AdjacencyFn& get_neighbors,
              uint32_t num_nodes,
    ...
    // Note: neighbor_buf is shared across recursion levels, so we need to copy
    // locally if we want to iterate after recursive calls. For IDA*, we can
    // iterate and recurse immediately.

    uint32_t local_neighbors[64];
    const uint32_t n_count = get_neighbors(current_id, local_neighbors, 64);

    for (uint32_t i = 0; i < n_count; ++i) {
      const uint32_t neighbor = local_neighbors[i];
