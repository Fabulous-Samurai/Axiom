...
class AXIOM_EXPORT LinearSystemParser : public IParser {
 ...
 private:
  ...
  bool ParseLinearSystem(std::string_view input, Matrix &A,
  ...
  std::optional<Vector> CramersRule(const Matrix &A,
  ...
  std::pair<Vector, Matrix> ComputeEigenvalues(
      const Matrix &A, int max_iterations = 100) const noexcept;
};
