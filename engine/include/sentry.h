...
class Sentry {
 public:
  enum class State {
    ...
    ANALYZING =
        2,  // Sentry checking context (Is it a heavy workload or a freeze?)
  ...
  /**
   * @brief Called by the hot-loop (Isolated Core) to signal it is alive and
   * working.
   */
 ...
 private:
  ...
  // Thresholds
  static constexpr uint64_t SUSPICION_THRESHOLD_MS =
      50;  // 50ms without UI heartbeat = Suspicious
  static constexpr uint64_t PANIC_THRESHOLD_MS =
      2000;  // 2s without UI heartbeat = Panic
};
