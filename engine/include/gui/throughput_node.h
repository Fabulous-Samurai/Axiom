...
public:
    ...
    void setColor(const QColor& c) { if (m_color != c) { m_color = c; emit colorChanged(); update(); } }
    
    int maxPoints() const { return m_max_points; }
    void setMaxPoints(int p) { if (m_max_points != p) { m_max_points = p; emit maxPointsChanged(); update(); } }
