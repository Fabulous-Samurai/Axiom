...
/**
 * @file ingress.cpp
 * @brief Implementation of Kernel-Bypass Ingress Factory and Concrete Providers (Operation VARIANT SHIFT)
 */
...
bool AF_XDPChannel::start(HarmonicArena* target_arena) noexcept {
    (void)target_arena;
    #ifdef __linux__
        std::cout << "[AXIOM Ingress] Initializing AF_XDP Zero-Copy Channel" << std::endl;
        return true;
    #else
        return false;
    #endif
...
size_t PosixUDPChannel::poll(std::function<void(const IngressFrame&)> callback) noexcept {
    (void)callback;
    return 0;
}
...
bool MockHFTChannel::start(HarmonicArena* target_arena) noexcept {
    (void)target_arena;
    std::cout << "[AXIOM Ingress] Initialized Mock HFT Feed Provider" << std::endl;
    return true;
}

void MockHFTChannel::stop() noexcept {}

size_t MockHFTChannel::poll(std::function<void(const IngressFrame&)> callback) noexcept {
    (void)callback;
    return 0;
}
